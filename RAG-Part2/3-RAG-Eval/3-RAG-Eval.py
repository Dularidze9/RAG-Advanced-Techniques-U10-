...
# --- Global Variables (Internal Module State) ---
DEVICE = None
...
def setup_embedding_model(model_name="sentence-transformers/all-MiniLM-L6-v2"):
    """Initialize dense embedding model and tokenizer"""
    ...
    try:
        ...
        print(f"Dense embedding model '{model_name}' loaded on {DEVICE}")
        return True
    except Exception as e:
        print(f"Error loading dense embedding model '{model_name}': {e}")
        return False
...
def generate_embedding_internal(text_list):
    ...
    if DENSE_TOKENIZER is None or DENSE_MODEL is None:
        if not setup_embedding_model():
            raise EnvironmentError("Dense model not initialized and failed to auto-setup. Please call setup_embedding_model() first.")
...
def chunk_text(text, chunk_size=150, overlap=50):
    """Split text into chunks and assign indices"""
    chunks = []
    start = 0
    text_len = len(text)
    index = 0
...
def initialize_retrievers(pdf_text_chunks):
    ...
    if not pdf_text_chunks:
        print("Error: Text chunk list is empty. Cannot initialize retrievers.")
        return None, None
    # Prepare text content for BM25
    ...
    if DENSE_MODEL is None:
        if not setup_embedding_model():
            print("Error: Failed to load embedding model, cannot initialize FAISS.")
...
def hybrid_retrieval(query_text, bm25_obj, faiss_idx, all_chunks, k_results=3, alpha=0.5):
    ...
    if not all_chunks:
        print("Warning: Text chunk list is empty, cannot perform retrieval.")
        return []
    ...
    if not use_bm25 and not use_faiss:
        print("Warning: Both BM25 and FAISS retrievers are unavailable.")
        return []
    ...
    if use_bm25:
        ...
        try:
            ...
            if len(bm25_scores_list) == len(all_chunks):
                ...
                print(f"Warning: BM25 returned {len(bm25_scores_list)} scores for {len(all_chunks)} chunks. Scores may be inaccurate.")
        except Exception as e:
            print(f"Error during BM25 retrieval: {e}. All BM25 scores set to 0.")
    ...
    if use_faiss:
        ...
        if num_dense_candidates > 0:
            ...
            print("Warning: FAISS candidate count is 0 or index empty. Skipping dense retrieval.")
...
def test_retrieval_system():
    """Test PDF text extraction, chunking, hybrid retrieval, and evaluation"""
    print("--- Starting Retrieval System Test (using PDF) ---")
    if not setup_embedding_model():
        print("Error: Failed to load embedding model. Test aborted.")
        return []
    pdf_path = PDF_PATH
    print(f"Loading PDF file: '{pdf_path}'")
    raw_text = extract_text_from_pdf(pdf_path)
    if not raw_text.strip():
        print(f"Failed to extract text from '{pdf_path}' or file not found. Test aborted.")
        return []
    text_chunks = chunk_text(raw_text, chunk_size=150, overlap=50)
    if not text_chunks:
        print("Failed to generate text chunks. Test aborted.")
        return []
    print(f"\nGenerated {len(text_chunks)} text chunks from '{pdf_path}'.")
    ...
    try:
        with open(output_txt_path, "w", encoding="utf-8") as f:
            for chunk in text_chunks:
                ...
                f.write(chunk['text'])
                f.write("\n\n--------------------\n\n")
        print(f"All text chunks saved to: {output_txt_path}")
    except Exception as e:
        print(f"Error saving text chunks: {e}")

    if text_chunks:
        print("First 3 text chunk examples:")
        for chunk in text_chunks[:3]:
            print(f"  Chunk {chunk['index']}: \"{chunk['text'][:100].replace(chr(10), ' ')}...\"")
    ...
    if bm25_retriever is None and (faiss_retriever is None or faiss_retriever.ntotal == 0):
        print("Failed to initialize retrievers or FAISS index empty. Test aborted.")
        return []

    sample_query = "What is an AI Agent?"
    print(f"\nRunning hybrid retrieval for query: \"{sample_query}\" (k=3, alpha=0.5)")
    ...
    if retrieved_documents:
        ...
        for doc in retrieved_documents:
            ...
            print(f"  Text: \"{doc['text'][:150].replace(chr(10), ' ')}...\"")
            print("-" * 20)
    else:
    ...
    print(f"Hit Rate: {eval_result['hit_rate']:.4f}")
    print(f"Recall: {eval_result['recall']:.4f}")
    print(f"Precision: {eval_result['precision']:.4f}")
    print(f"Matched: {eval_result['matched']}")
    print(f"Keyword Match Rate: {eval_result['keyword_match_rate']:.4f}")
